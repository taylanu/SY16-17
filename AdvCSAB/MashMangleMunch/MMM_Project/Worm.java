 ...
 //Entity <- Player <- (abstract) Monster <- Worm
 //WORMOID EXTRA CREDIT
 
public class Worm extends Monster
{
 //ARGS:  row, col, image collection
   public Worm(int r, int c, String[][][] image)
   { 
      super("Wormoid", r, c, image, 10, 30, 0, 100, 5, "SHRIEK", 30);
   ...
   public Worm(String n, int r, int c, String[][][] image, int sp, int spp, int rt)
   {
      ...
      if(index >= 0)
      {
         ...
         if(contents[index].startsWith("CROWD"))		//more health for eating a crowd of people
         {
            ...
            if(contents[index].endsWith("bus"))			//satisfy hunger more for eating a bus
            {
               ...
               if(contents[index].endsWith("bomber"))			//satisfy hunger more for eating a bomber
               {
                  this.setHealth(this.getHealth()+5);
