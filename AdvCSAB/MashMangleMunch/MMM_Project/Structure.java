...
public class Structure extends Entity {
    ...
    public Structure(String n, int r, int c, int p, String[][][] image, int ad, boolean iP, boolean iD, int ht, int h, int SIZE, int ii, long pv) {
        ...
        else if (super.getName().endsWith("LANDMARK") && height == 2)
            heightDiff = (int) (SIZE / 1.0);
        else if (super.getName().equals("ELEC TOWER 0"))
            heightDiff = (int) (SIZE / 1.5);
        else if (super.getName().startsWith("WATER TOWER"))
            heightDiff = SIZE / 2;
        else if (super.getName().equals("ELEC TOWER 1") || super.getName().startsWith("Blop"))
            heightDiff = 0;
        else
    ...
    @Override
    public String toString() {    //used to store structures in a file for custom built maps
    ...
    public boolean canShootOver(Bullet b) {
        if (b.inAir())
            return true;
        if (super.getName().startsWith("Blop")) { //shells and fire will damage and destroy Blop-glop
