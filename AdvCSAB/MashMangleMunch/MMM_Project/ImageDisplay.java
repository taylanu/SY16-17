...
public class ImageDisplay extends MMMPanel {
    ...
    public static void loadImages() {
        ...
        for (int name = 0; name < playerImages.length; name++)
            for (int dir = UP; dir <= LEFT; dir++)
                for (int frame = 0; frame < 4; frame++)
                    for (int head = UP; head <= LEFT; head++) {    //there is no image where head is backwards fireciton from body direction, so just show the one where they are facing the same direction
        ...
        for (int dir = UP; dir <= LEFT; dir++) {    //vehicles with no animation frames and no head direction changes
        ...
        for (int frame = 0; frame < 2; frame++)
            for (int dir = UP; dir <= LEFT; dir++) {    //vehicles with 2 animation frames and no head direction changes
        ...
        for (int dir = UP; dir <= LEFT; dir++)
            for (int head = UP; head <= LEFT; head++) {    //vehicles with no animation frames and 4 head direction changes
        ...
        for (int frame = 0; frame < 4; frame++)
            for (int dir = UP; dir <= LEFT; dir++) {    //units with 4 animation frames and no head direction changes
        ...
        for (int dir = UP; dir <= LEFT; dir++) {    //for projectiles with 4 directions and no animaiton frames
        ...
        for (int dir = UP; dir <= LEFT; dir++)
            for (int frame = 0; frame < 4; frame++) {//for projectiles with 4 directions and 4 animation frames
        ...
        for (int i = 0; i < 2; i++)
            for (int frame = 0; frame < 4; frame++) {  //for structures with 2 variations and 4 animation frames
        ...
        for (int i = 0; i < 2; i++)
            for (int frame = 0; frame < 2; frame++) {  //for structures with 2 variations and 2 animation frames
                fuelDepotImages[i][frame][0] = "images/structures/gasStations/fuelTank" + i + frame + dayVersion + ".GIF";
            }

        for (int i = 0; i < 3; i++)
            for (int frame = 0; frame < 2; frame++) {  //for structures with 3 variations and 2 animation frames
                gasStationImages[i][frame][0] = "images/structures/gasStations/gasStation" + i + frame + ".GIF";
            }

        //suburb buildings
        for (int i = 0; i < 4; i++)
            for (int frame = 0; frame < 4; frame++) {  //for structures with 4 variations and 4 animation frames
        ...
        //skyscrapers
        for (int i = 0; i < 6; i++)
            for (int frame = 0; frame < 4; frame++) {  //for structures with 6 variations and 4 animation frames
        ...
        //landmarks
        for (int i = 0; i < 10; i++)
            for (int frame = 0; frame < 4; frame++) {  //for structures with 10 variations and 4 animation frames
                    ...
                    if (r == curr.getRow() && c == curr.getCol()) {
                        ...
                        {                    //don't advance animation bc we are not moving - subtract jump altitude if we are jumping
                            ...
                            if (curr.isMovingLeft() || curr.isMovingUp()) {
                                ...
                                else if (board[r][c][panel].equals("~~~"))        //we are in water - draw a 1/2 water on top of player
                                {
                                    int rand = (int) (Math.random() * 4);
            ...
            for (int c = 0; c < structures[0].length; c++) {
                ...
                for (int i = 0; i < players.length; i++) {
                    ...
                    if (pl == null || pl.getName().equals("NONE"))        //player 2 might not be activated
                        continue;
                    if (pl.getName().indexOf("firetruck") >= 0) {    //put out any fire close to the firetruck, and spray away any Blop-glop
                    ...
                    int pC = pl.getCol();
                    if (curr != null && curr.getHealth() <= 0) {
                        if (pR == r && pC == c && pl.isMovingDown()) {//if a structure is rubble, we want to  show the player over it when they are on the same position and moving down
                            closePlayerFound = true;
                            break;
                        }
                        if (pR == r + 1 && pC == c && (pl.isMovingUp() || jumpAltitude != 0)) {//we want ot show the player over the rubble if they are one row below it and moving up
                            closePlayerFound = true;
                            break;
                        }
                    }
    ...
    public static void selectGame(Graphics g) {
        ...
        if (highlightArea == 1) {
            ...
            } else if (gameMode == EARTH_INVADERS) {
    ...
    public static void displayStats(Graphics g, int x, int y, int width, int height) {
        ...
        if (gameStarted) {
            ...
            if (p1partner) {
                ...
                if (p2name.equals("BoobooTron"))    //make the p1partner name fit
                    p2name = "BooTron";
                else if (p2name.equals("King-Clunk"))    //make the p1partner name fit
                    p2name = "Clunk";
                else if (p2name.equals("The-Blop"))    //make the p1partner name fit
                    p2name = "Blop";
                else if (p2name.equals("Gobzilly"))    //make the p1partner name fit
                    p2name = "Zilly";
                else if (p2name.equals("WoeMantis"))    //make the p1partner name fit
                    p2name = "Woe";
            ...
            } else if (health > 0 && numFrames <= messageTime + messageDuration) {
                g.setFont(new Font("Monospaced", Font.ITALIC, size));
                g.setColor(Color.red);
        ...
        if (!gameStarted && !titleScreen && (highlightArea >= 0 && highlightArea <= 23) && gameMode != CITY_SAVER) {    //show narrative of monster that is moused over
            ...
            if (!toggleMap || gameMode == CITY_SAVER || gameMode == EARTH_INVADERS) {
                ...
                if (gameMode == CITY_SAVER)
                    y += (int) (size * .45);
                else if (players[PLAYER1].isFlyer())
                    g.drawString("Fly/land   : SPACE", x, y += (int) (size * .45));
                else if (players[PLAYER1].isDigger())
        ...
        if (!gameStarted) {//***SELECT DIFFICULTY or show title screen info
            y += (size);
            g.setFont(new Font("Monospaced", Font.BOLD, (int) (size * .75)));
            g.setColor(Color.yellow.darker().darker());
                ...
                if (gameStarted) {
                    ...
                    if (waterPanels != null && waterPanels.size() > 0) {
                        ...
                        if (waterPanels.contains(((c * panelState.length) + r))) {
                            ...
                            } else {                //draw half block with water
    ...
    public static void drawMainScreen(Graphics g) {
        ...
        if (titleScreen) {
            ...
            if (nuked && nukeSize < ((board[0].length * cellSize)) / 2) {
                ...
                nukeSize++;
            } else if (monsterWins && nukeSize < ((board[0].length * cellSize)) / 2) {    //when the monster wins, show a monster growing out of the center of the screen
                Player curr = players[PLAYER1];
                int x = (((board[0].length * cellSize)) / 2) - (nukeSize);
                int y = (((board.length * cellSize)) / 2) - (nukeSize);
                int diam = nukeSize;
            ...
            if (monsterWins)
                ...
                if (numFrames <= warningTime + warningDuration && gameStarted && numFrames > 500) {
                    if (numFrames % 2 == 0)
                        ...
                        g.setColor(Color.red);
                } else if (gameMode == CITY_SAVER || gameMode == EARTH_INVADERS)
                    g.setColor(Color.red.darker().darker().darker());
                else if (numFrames <= painTime + painDuration && gameStarted)
                ...
                if (panel >= 0 && panel <= 2 && gameStarted)                                //make border red if you are at a boundry
                {
                    if (numFrames <= warningTime + warningDuration && gameStarted && numFrames > 500) {
                        ...
                        g.setColor(Color.red.darker().darker().darker());
                    else if (numFrames <= painTime + painDuration)
                        g.setColor(Color.red);
                    else
                        g.setColor(Color.red.darker().darker().darker());
                }
