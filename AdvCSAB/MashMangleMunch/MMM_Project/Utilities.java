...
//post: changes head direction of player curr
    ...
    public static void absorbElecTower(Monster curr) {
        ...
        } else if (bDir == RIGHT && c < board[0].length - 1) {
        ...
        } else if (bDir == DOWN && r < board.length - 1) {
            Structure str = structures[r + 1][c][panel];
            if (str != null && str.getName().startsWith("ELEC")) {
                int healthBoost = (int) (Math.random() * 11) + 5;
                if (str.getName().endsWith("0"))    //large tower
    ...
    public static void grab(Monster curr) {
        ...
        for (int i = 1; i < players.length; i++) {
            ...
            } else if (curr.canGrabUnit(name))                    //check to see if we can grab a unit of this type (in Monster.java)
            {
                message = "Gotcha!";
                messageTime = numFrames;
                if (curr.getHealth() > 0)
...
//post:  for a shooting vehicle within one cell of curr, set their mind control status to true so that they attack teammates
    ...
    public static void monsterStomp(Monster curr, int playerIndex) {
        ...
        if (curr.isFlyer() && gameMode != EARTH_INVADERS) {
            ...
            if (!curr.isFlying())                                    //only stomp if we are landing
            {
                ...
                if (board[r][c][panel].startsWith("#"))        //landing on electric lines or power station - instant death
                {
                    ...
                    if (board[r][c][panel].startsWith("S~") || board[r][c][panel].equals("TU2") || board[r][c][panel].equals("TR2")) {//if we land on a bridge, destroy it (change it to water)
                        board[r][c][panel] = "~~~";
                ...
                if (str.getHealth() == 0) {
                    ...
                    if (players[PLAYER1].getHealth() > 0) {
                        ...
                        if (panel == 4)        //we only protect the center panel in CITY_SAVER
                            propertyValue -= value;
                        updateKillStats(str);
                        if (str.getName().endsWith("LANDMARK")) {    //if we destroyed a landmark, elevate the threat level
                            if (threatLevel < 4)
                                threatLevel++;
                        }
                    }
        ...
        if (board[r][c][panel].startsWith("S~") || board[r][c][panel].equals("TU2") || board[r][c][panel].equals("TR2")) {//if we stomp a bridge, destroy it (change it to water)
            board[r][c][panel] = "~~~";
            if (structures[r][c][panel] != null)    //there might be Blop-Glop there to remove
                structures[r][c][panel] = null;
            if (players[PLAYER1].getHealth() > 0)
    ...
    public static void monsterEat(Monster curr) {
        ...
        if (!contents[0].equals("empty"))
            ...
            index = 1;
        if (curr.isAmbidextorous()) {    //if they are holding a CROWD, prioritize eating them
...
//post:add to kill count for each unit type - crowds, cars, boats, aircraft, buildings and possibly update the threat level
    public static void updateKillStats(Entity curr) {
        ...
        if (name.startsWith("CROWD"))
            killStats[0]++;
        else if (name.startsWith("CYCLE") || name.startsWith("CAR") || name.startsWith("TRAIN") || name.startsWith("TANK"))
            killStats[1]++;
        else if (name.startsWith("BOAT"))
            killStats[2]++;
        else if (name.startsWith("AIR"))
            killStats[3]++;
        else if (curr instanceof Structure) {
...
//			any player 2 monsters are removed if they die
    public static void updateMonsterStatus(int index) {
        ...
        if (gameMode == CITY_SAVER) {
            ...
            for (int p = 0; p < numStructures.length; p++) {    //check all panels to see if we clear one (because we might be in another panel when it reaches the level of uninhabitabe because of fire)
                if (numStructures[p] >= 0 && numStructures[p] <= 2) {    //we rendered a panel uninhabitable - give monster a bonus
        ...
        if ((index == PLAYER2 || index == AIPLAYER || index == BLOPSPLIT) && curr.getHealth() <= 0)
            currDies = true;
        else if (gameMode == CITY_SAVER && index >= AIMONSTER1 && index <= AIMONSTER4 && curr.getHealth() <= 0)
            currDies = true;
        if (currDies) {    //p2 monster partner is dead - remove them
            ...
            } else if (p3toggle && index == AIPLAYER) {
                players[index] = new Player();
                p3toggle = false;
                message = "Partner Dies!";
            } else if (blopSplit && index == BLOPSPLIT) {
                players[index] = new Player();
                blopSplit = false;
                message = "Twin Dies!";
            } else if (gameMode == CITY_SAVER && index >= AIMONSTER1 && index <= AIMONSTER4) {
                players[index] = new Player();
                numMonstersKilled++;
        ...
        else if (gameMode == MONSTER_MASH && numFrames % energyTime == 0) {
        ...
        } else if (numFrames % hungerTime == 0) {
        ...
        if (board[curr.getRow()][curr.getCol()][panel].startsWith("~") && !curr.isFlying())    //we are in the water
        {
            if (curr.damageInWater() && numFrames % 250 == 0)    //gorilla, bug and robot lose health in water
            {
                ...
                messageTime = numFrames;
                painTime = numFrames;
            } else if (curr.energyAbsorber() && numFrames % 125 == 0) {
    ...
    public static void findCompleteCities() {
        for (String fname : mapList) {
            if (fname.length() > 1) {
                ...
                if (Character.isDigit(last)) { //record all of the panel numbers recorded at the end of each map that starts with this name
