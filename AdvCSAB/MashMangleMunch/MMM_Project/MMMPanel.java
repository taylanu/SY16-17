...
//we need an array of these ArrayLists to store locations across multiple panels (of which there are 9)
    ...
    public static void resetGame() {
        ...
        for (int i = 0; i < webs.length; i++) {    //collection of row & col values [r1,c1 to r2,c2] for each panel - web locations between buildings
    ...
    public static void monsterCollidesVehicle(Monster curr, int i) {
        ...
        if (Utilities.distance(x1, y1, x2, y2) <= (cellSize / 2)) {
            ...
            if (name.startsWith("CROWD")) {
                ...
                if (name.equals("TRAIN engine")) {//the engine is destroyed, so destroy the rest of the train
                ...
                } else if (name.equals("TRAIN car")) {//the car is destroyed, so destroy the caboose
                    trainInPanel[panel] = false;
                    for (int tp = TRAIN1; tp <= TRAIN3; tp++) {
    ...
    public static void checkCollisions() {
        for (int i = 0; i < players.length; i++) {
            ...
            for (int j = 0; j < bullets.size(); j++)            //see if bullets hit any enemies
            {
                ...
                int sX = shot.getX();
                int sY = shot.getY();
                if (Utilities.distance(sX, sY, x2, y2) <= (cellSize / 2)) {        //if bullet is an air shot and target is not an airplane nor one of the monsters
                    if (shot.inAir() && !curr.isFlying() && (curr instanceof Vehicle)) {  //air shots can only hit aircraft
                        continue;
                    }
                    ...
                    if (shot.getType().startsWith("SHRIEK") && !curr.isFlying())
                        explosions.add(new Explosion("SMALL", x2 - (cellSize / 2), y2 - (cellSize / 2), puffImages, animation_delay));
                    else if (shot.getType().startsWith("BULLET")) {
                        if (curr.imperviousToBullets()) {
                            continue;
                        }
                        explosions.add(new Explosion("SMALL", x2 - (cellSize / 2), y2 - (cellSize / 2), puffImages, animation_delay));
                    } else if (curr.isFlying()) {
                        channels[0].programChange(instr[GUNSHOT].getPatch().getProgram());
                        channels[0].noteOn((int) (Math.random() * 6) + 30, (int) (Math.random() * 10) + 50);
                        explosions.add(new Explosion("BIG", x2 - (cellSize / 2), y2 - (cellSize / 2), explosionImages, animation_delay));
                    } else if (name.startsWith("BOAT")) {
    ...
    public static void playerShoot(Player current, int owner) {
        ...
        if (gameMode != EARTH_INVADERS)        //we always want to shoot a projectile in earth invaders mode
        {
            if (curr.isMindControl())            //scramble the brains of any Vehicle in close proximity
            {
                ...
                return;
            } else if (curr.canSplit())               //splitters doesn't shoot, but can separate for the price of health
            {
                splitTheBlop(curr); //this call signifies that I have to modify the above method.
                return;
            } else if (!curr.isShooter())        //if we are not a shooter, nor a mind controller nor a splitter, end the method
        ...
        if (gameMode == EARTH_INVADERS) {    //everyone can shoot in EARTH_INVADERS, so reload if time - don't shoot and show message
            if ((numFrames > curr.getReloadTime() && numFrames < curr.getLastShotTime() + curr.getReloadTime()))
                needToReload = true;
        } else if (curr.isThrower()) {    //Throwers have to have ammo in hand, otherwise, don't shoot and show message
            if (curr.projectileType().equals("empty"))
                needToReload = true;
        } else {    //non-throwers in non-EARTH_INVADERS mode might have reload time
            if ((numFrames > curr.getReloadTime() && numFrames < curr.getLastShotTime() + curr.getReloadTime()))
                needToReload = true;
        }
        if (needToReload) {
        ...
        if (gameMode == EARTH_INVADERS || (curr.isThrower() && !curr.projectileType().equals("empty")) || (curr.getHunger() < hungerLevels.length - 1 || curr.energyAbsorber())) {//always allowed to shoot in earth invaders mode, otherwise, shooter monster types only shoot if they are not weakened from hunger
            ...
            } else if (type.equals("GLOP")) {
                channels[0].programChange(instr[GUITAR_FRET_NOISE].getPatch().getProgram());
                channels[0].noteOn((int) (Math.random() * 6) + 45, (int) (Math.random() * 10) + 30);
                temp = new Bullet("glop" + hd, curr.getX(), curr.getY(), 15, blopGlopImages, SPEED, type, SPEED * 8, airShot, owner, -1, -1);
            } else if (curr.isThrower() && ((!type.equals("empty") && !type.equals("CROWD")) || gameMode == EARTH_INVADERS)) {
                channels[0].programChange(instr[GUITAR_FRET_NOISE].getPatch().getProgram());
                channels[0].noteOn((int) (Math.random() * 6) + 35, (int) (Math.random() * 10) + 30);
                int speed = SPEED * 4;
                if (gameMode == EARTH_INVADERS) {
                    ...
                    if (!contents[0].startsWith("CROWD") && !contents[0].startsWith("empty"))
                        contents[0] = "empty";
                    else if (!contents[1].startsWith("CROWD") && !contents[1].startsWith("empty"))
                        contents[1] = "empty";
...
//post:directs monster or vehicle depending on keyboard input k
    public static void playerMove(int k, int playerIndex) {
        ...
        {    //stomp command for p1 || p2
            ...
            {
                if (!curr.isFlying() && (numFrames > curr.getReloadTime() && numFrames < curr.getLastShotTime() + curr.getReloadTime())) {//use the same stamina that we use to keep breath attack from being spammed
                    message = curr.reloadingMessage();
                    messageTime = numFrames;
                } else {
        ...
        } else if (k == KeyEvent.VK_UP || k == KeyEvent.VK_W)    //we can't walk from under an impassable structure to behind it (unless we destroy it first, or we are flying over or digging under it)
        {    //up command for player 1 || player 2
            ...
            //While underground or flying, a monster should be able to traverse to a location above or below her even if there is a structure there.
            {
                if (r == 1)        //top row
                {
                    ...
                    if (gameMode != CITY_SAVER && gameMode != EARTH_INVADERS && playerIndex == 0 && panel > 2 && (!p1partner || (p1partner && players[PLAYER2].getRow() == r))) {    //if there isn't a monster partner OR there is, and they are in the same row, then change panels
        ...
        } else if ((k == KeyEvent.VK_DOWN || k == KeyEvent.VK_S) && gameMode != EARTH_INVADERS)//we can not move down in this mode
        {    //down command for player 1 || player 2
            ...
            //While underground or flying, a monster should be able to traverse to a location above or below her even if there is a structure there.
            {
                ...
                {    //can't move down in panels 6, 7 or 8
                    if (gameMode != CITY_SAVER && gameMode != EARTH_INVADERS && playerIndex == 0 && panel < 6 && (!p1partner || (p1partner && players[PLAYER2].getRow() == r))) {    //if there isn't a monster partner OR there is, and they are in the same row, then change panels
                        panel += 3;
                        ceaseFireTime = numFrames;
                        curr.setRow(1);
                        Spawner.initialVehicleSpawn(panel);
        ...
        } else if (k == KeyEvent.VK_LEFT || k == KeyEvent.VK_A)    //we can walk behind structures from the left and right
                ...
                {    //can't move left in panels 0, 3 or 6
                    ...
                    } else// if(!((p3toggle && playerIndex==2) ||  (blopSplit && playerIndex==3) || (gameMode==CITY_SAVER && monsterIndex.contains(playerIndex))))
                    {
                        if (p1partner && players[PLAYER1].getCol() != players[PLAYER2].getCol() && panel != 0 && panel != 3 && panel != 6) {
                            message = "Wait for friend!";
                            messageTime = numFrames;
                        } else if ((panel == 0 || panel == 3 || panel == 6) && playerIndex == 0) {
                            message = "Blocked!";
                            messageTime = numFrames;
                        }
                    }
                ...
                {    //can't move right in panels 2, 5 or 8
                    ...
                    } else// if(!((p3toggle && playerIndex==2) ||  (blopSplit && playerIndex==3) || (gameMode==CITY_SAVER && monsterIndex.contains(playerIndex))))
                    {
                        if (p1partner && players[PLAYER1].getCol() != players[PLAYER2].getCol() && panel != 2 && panel != 5 && panel != 8) {
                            message = "Wait for friend!";
                            messageTime = numFrames;
                        } else if ((panel == 2 || panel == 5 || panel == 8) && playerIndex == 0) {
                            message = "Blocked!";
                            messageTime = numFrames;
                        }
                    }
    ...
    public static void movePlayerSmoothly() {
        for (int i = 0; i < players.length; i++) {
            ...
            if (Math.abs(curr.getMoveIncrX()) >= cellSize || Math.abs(curr.getMoveIncrY()) >= cellSize) {
                if (curr.isMovingUp() && r > 0 && (!board[r - 1][c][panel].startsWith("#") || curr.isFlying() || curr.isDigging()))
                    curr.setRow(r - 1);
                else if (curr.isMovingDown() && r < board.length - 1 && (!board[r + 1][c][panel].startsWith("#") || curr.isFlying() || curr.isDigging()))
                    curr.setRow(r + 1);
                else if (curr.isMovingLeft() && curr.getCol() > 0 && (!board[r][c - 1][panel].startsWith("#") || curr.isFlying() || curr.isDigging()))
                    curr.setCol(c - 1);
                else if (curr.isMovingRight() && c < board[0].length - 1 && (!board[r][c + 1][panel].startsWith("#") || curr.isFlying() || curr.isDigging()))
                    curr.setCol(c + 1);
                curr.clearDirections();
            ...
            if (curr.getName().indexOf("firetruck") >= 0) {    //put out any fire close to the firetruck, and spray away any Blop-glop
...
//			keeps the player in the bounds of the size of the array board
    public void change(int k) {
        ...
        if (k == KeyEvent.VK_ESCAPE)                        //quit
        {
            if (scoresUpdated) {
                ...
                else if (gameMode == BOMBER_DODGER)
        ...
        if (gameStarted) {
            ...
            if (!(curr.isMoving() || (curr instanceof Monster && ((Monster) (curr)).isJumping())) && (!Utilities.p2Command(k))) {  //if we are not in the middle of a move, we are a monster that is not jumping and we did not issure a command for player 2
                playerMove(k, 0);
                return;
            }
            if (p2toggle || p1partner) {
                ...
                {                    //move up
                    if (k == KeyEvent.VK_W)      //we can't walk from under an impassable structure to behind it (unless we destroy it first)
                    {
                        if ((curr2.getBodyDirection() == UP && r2 > 1 && MapBuilder.noStructure(r2 - 1, c2, panel))) {
                            ...
                            if (str != null && str.getName().startsWith("Blop") && str.getHealth() != 0 && !curr2.getName().endsWith("tank") && !curr2.getName().endsWith("flame")) {//if we are not a tank or flame tank and there is Blop-glop above us, don't move into it (just change orientation)
                                curr2.setBodyDirection(UP);
                                curr2.setHeadDirection(UP);
                            } else if (str != null && str.getName().equals("hole") && str.getHealth() != 0) {//if there is a hole there, don't move into it (just change orientation)
                                curr2.setBodyDirection(UP);
                                curr2.setHeadDirection(UP);
                            } else
                    ...
                    } else if (k == KeyEvent.VK_S)        //move down
                    {
                        if ((curr2.getBodyDirection() == DOWN && r2 < board.length - 2 && MapBuilder.noStructure(r2, c2, panel))) {
                            Structure str = structures[r2 + 1][c2][panel];
                            if (str != null && str.getName().startsWith("Blop") && str.getHealth() != 0 && !curr2.getName().endsWith("tank") && !curr2.getName().endsWith("flame")) {//if we are not a tank or flame tank and there is Blop-glop below us, don't move into it (just change orientation)
                        ...
                        {                                //so we dont have to check to see if there is a structure here
                            if ((curr2.getBodyDirection() == LEFT && c2 > 1))// || curr2.head360())
                            {
                                ...
                                if (str != null && str.getName().startsWith("Blop") && str.getHealth() != 0 && !curr2.getName().endsWith("tank") && !curr2.getName().endsWith("flame")) {//if we are not a tank or flame tank and there is Blop-glop left of us, don't move into it (just change orientation)
                        ...
                        } else if (k == KeyEvent.VK_D)            //move right
                        {
                            if ((curr2.getBodyDirection() == RIGHT && c2 < board[0].length - 2))// || curr2.head360())
                            {
                                Structure str = structures[r2][c2 + 1][panel];
                                if (str != null && str.getName().startsWith("Blop") && str.getHealth() != 0 && !curr2.getName().endsWith("tank") && !curr2.getName().endsWith("flame")) {//if we are not a tank or flame tank and there is Blop-glop RIGHT of us, don't move into it (just change orientation)
                ...
                {
                    if (!(curr2.isMoving() || (curr2 instanceof Monster && ((Monster) (curr2)).isJumping())) && (Utilities.p2Command(k))) {    //if we are not in the middle of a move, we are a monster that is not jumping and issued a player 2 command
                        playerMove(k, 1);
                        return;
                    }
            ...
            if (needToStart) {
                if (gameMode == CITY_SAVER) {
                    ...
                    if (players[PLAYER1].healInWater()) {
                        forceWater = true;
                    } else if (players[PLAYER1].energyAbsorber()) {
                        forceTracks = true;
                    }
                }
                ...
                if (players[PLAYER1].healInWater()) {    //...now that the map is generated, drop player into a spot with water
                    if (waterPanels.size() > 0) {
                        ...
                        for (int r = 1; r < board.length - 1; r++)                //we don't want to add the borders because they are hidden
                            for (int c = 1; c < board[0].length - 1; c++)
                                if (board[r][c][panel].equals("~~~") && (r == board.length / 2 || c == board[0].length / 2)) {    //add water cells that are in the center of the board
                                    int[] coord = new int[2];
                                    coord[0] = r;
                                    coord[1] = c;
                                    coords.add(coord);
    ...
    public void mouseMoved(MouseEvent e) {
        ...
        if (gameStarted) {
            ...
            if (horizDiff == vertDiff && horizDiff == 0)
                cursorIndex = 4;        //center cursor
            else if (Math.abs(horizDiff) < Math.abs(vertDiff)) {
                if (vertDiff < 0)
                    cursorIndex = UP;
                else
                    cursorIndex = DOWN;
            ...
            if (curr.getMoveIncrY() != 0 || curr.getMoveIncrX() != 0 || pause)
                return;
            if (horizDiff == vertDiff && horizDiff == 0) {
            } else if (Math.abs(horizDiff) < Math.abs(vertDiff)) {    //move up or down
                if (vertDiff < 0) {
    ...
    public void mouseWheelMoved(MouseWheelEvent e) {
        if (gameStarted && !pause) {
            ...
            } else if (notches >= WHEEL_SENSITIVITY && numFrames > curr.getHeadTurnTime()) {
