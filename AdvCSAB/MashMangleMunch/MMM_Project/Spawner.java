...
public class Spawner extends MMMPanel {
    ...
    public static ArrayList<String> findAvailableUnits() {
        ...
        if (threatLevel >= 1) {//***POLICE*****
            units.add("CAR police");
            units.add("CAR police");
        }
        if (threatLevel >= 2) {//***NATL GUARD*****
    ...
    public static void initialVehicleSpawn(int p) {
        ...
        if (p2toggle && !players[PLAYER2].getName().equals("NONE"))        //there is a player 2 - relocate them
        {
            ...
            } else if (name.equals("TANK flame")) {
                if (p2TankSpawnPoints[panel].size() > 0) {
                    rand = (int) (Math.random() * p2TankSpawnPoints[panel].size());
                    coord = p2TankSpawnPoints[panel].get(rand);
                } else        //backup - no spawns, so pick a random loc
            ...
            } else if (name.equals("TANK missile")) {
                if (p2CarSpawnPoints[panel].size() > 0) {
                    rand = (int) (Math.random() * p2CarSpawnPoints[panel].size());
                    coord = p2CarSpawnPoints[panel].get(rand);
                } else        //backup - no spawns, so pick a random loc
            ...
            } else if (name.equals("TANK artillery")) {
                if (p2CarSpawnPoints[panel].size() > 0) {
                    rand = (int) (Math.random() * p2CarSpawnPoints[panel].size());
                    coord = p2CarSpawnPoints[panel].get(rand);
                } else        //backup - no spawns, so pick a random loc
            ...
            } else if (name.equals("CYCLE police")) {
                if (p2CarSpawnPoints[panel].size() > 0) {
                    rand = (int) (Math.random() * p2CarSpawnPoints[panel].size());
                    coord = p2CarSpawnPoints[panel].get(rand);
                } else        //backup - no spawns, so pick a random loc
        ...
        for (int r = 0; r < board.length; r++) {
            for (int c = 0; c < board[0].length; c++) {
                ...
                spot[0] = r;
                spot[1] = c;
                if (board[r][c][p].startsWith("T")) {//tracks  - we want to start trains away from the edges because they will have multiple cars
                ...
                } else if (board[r][c][p].startsWith("~"))    //water
                {
                    boatSpawns.add(spot);
                    isWaterInPanel = true;
                } else                                            //not water
        ...
        if (gameMode == CITY_SAVER) {
            ...
            if (difficulty == MEDIUM)    //2 monsters for MEDIUM
                numMonsters = 2;
            else if (difficulty == HARD)    //3 monsters for HARD
                numMonsters = 3;
            else if (difficulty == NIGHTMARE)    //4 monsters for NIGHTMARE
                numMonsters = 4;
            for (int m = AIMONSTER1, nm = 0; m <= AIMONSTER4 && m < players.length && nm < numMonsters; m++, nm++) {  //AI monsters inhabit indexes 4,5,6 & 7
        ...
        for (int i = FIRST_VEHICLE; i < players.length; i++) {
            ...
            if (fireInPanel > 0 && !firetruck && rnd < .75)
                ...
                if (coord[0] == board.length / 2 || coord[1] == board[0].length / 2)
                    for (int numTries = 0; numTries < 1000; numTries++) {    //we don't want bombers to spawn in the same row/col as the monster when we start
                        if (coord[0] != board.length / 2 && coord[1] != board[0].length / 2)
                            break;
                        coord = getRandomSpawn();
                    }
    ...
    public static void resetEnemy(int i) {
        ...
        } else if (name.startsWith("BOAT"))    //a boat can be destroyed and try to respawn as we change panels, so if so, make it a car or bus instead
        {
        ...
        } else if (name.startsWith("TRAIN"))    //a train can be destroyed and try to respawn as we change panels, so if so, make it a car or bus instead
        {
        ...
        } else if (name.startsWith("AIR") && airSpawnPoints[panel].size() > 0) {
            rand = (int) (Math.random() * airSpawnPoints[panel].size());
            coord = airSpawnPoints[panel].get(rand);
        } else if ((name.equals("TANK tank") || name.equals("TANK flame")) && tankSpawnPoints[panel].size() > 0) {
            rand = (int) (Math.random() * tankSpawnPoints[panel].size());
            coord = tankSpawnPoints[panel].get(rand);
        } else if (vehicleSpawnPoints[panel].size() > 0) {
            rand = (int) (Math.random() * vehicleSpawnPoints[panel].size());
            coord = vehicleSpawnPoints[panel].get(rand);
        } else        //backup - no spawns, so pick a random loc
        {
        ...
        players[i].setCol(c);
        if (players[i].getName().startsWith("TRAIN")) {                                            //we want trains to follow the right wall to complete their track circuit
            ...
            if (name.endsWith("nukebomber"))    //if it is a bomber, set the coordinates for the nuke to be triggered
            {
                ...
                if (Math.random() < .5)        //set col to one of the two monsters (maybe one off to the left or right)
                {
                    if (bCol > 1 && Math.random() < .5)
                        bCol--;
                    else if (bCol < board[0].length - 1 && Math.random() < .5)
                        bCol++;
                }
            ...
            if (name.endsWith("nukebomber")) {
                ...
                if (Math.random() < .5)        //set col to one of the two monsters (maybe one off to the left or right)
                {
                    if (bRow > 1 && Math.random() < .5)
                        bRow--;
                    else if (bRow < board.length - 1 && Math.random() < .5)
                        bRow++;
                }
