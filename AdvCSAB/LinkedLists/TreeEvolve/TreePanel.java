...
public class TreePanel extends JPanel
{
   ...
   public TreePanel()
   {
      ...
      for(int i=0; i<forest.length; i++)
         forest[i] = forest[i].makeRandomTree();
   ...
   private class Listener implements ActionListener
   {
      ...
      {
         if(evolve)//considering that evolution is true.
         {
			 for(int i = 1;i<forest.length;i++){
				 
			 forest[i] = forest[0].mutate(MUTATION_RATE);
		}
			 numGenerations++;
			 int index = 0;
			 for(int i = 0;i<forest.length;i++){
			 if((forest[i].compareTo(forest[index]))>0){
			index = i; //finding index of most fit.
		}
				 Tree temp = forest[0];		 
				 forest[0] = forest[index];
				 forest[index] = temp;
         ...
         /*********************************************************
         WE WANT TO ADVANCE TO THE NEXT GENERATION IN HERE
         ********************************************************/
            repaint();
         }
      }
...
*/
   ...
   */
      public void keyPressed(KeyEvent e)
      {
         ...
         if(e.getKeyCode()==KeyEvent.VK_SPACE)	//advance one generation
         {
			 for(int i = 0; i < forest.length; i++){
				 Tree mutatedTree = forest[0].mutate(MUTATION_RATE);
				 if(mutatedTree.compareTo(forest[0])>0){// Greater than zero (positive) means that mutated is more fit than previous.
					 forest[0] = mutatedTree;
				 }
			 }
			 numGenerations++;
            repaint();
         ...
         if(e.getKeyCode()==KeyEvent.VK_1)      //Sample Size (2-25)
         {
            ...
            Tree[] tempforest = new Tree[forest.length-1]; //swap to create new forest and decrease size.
            //copy using for loop.
            for(int i = 0; i< forest.length; i++){
				tempforest[i] = forest[i];
			}
            //tempforest.Syarraycopy(forest);
            forest = tempforest;
            
            //forest.arraycopy
         /*********************************************************
         WE WANT TO DECREASE forest by 1
         ********************************************************/
            repaint();
            return;
         }
         if(e.getKeyCode()==KeyEvent.VK_2)      //Sample Size (2-25)
         {
            ...
            //tempforest.arraycopy(forest);
            forest = tempforest;//final assignment to ensure that forest is reassigned
         /*********************************************************
         WE WANT TO INCREASE forest by 1
         ********************************************************/
            repaint();
            return;
         }
